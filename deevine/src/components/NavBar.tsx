...
const NavBar: React.FC<ChildProps> = (props) => {
  ...
  return (
    <div className="h-11   ">
      <div
        className={`bg ${
          setMode ? " bodyBg fixed  z-50 w-full" : "lgBg fixed  z-50 w-full"
        } `}
      >
        <nav className="px-6 py-4       relative  bg-transparent  rounded-xl shadow-lg flex items-center  justify-between ">
          <div
            className={` bg  ${
              setMode
                ? "purp logo flex items-center cursor-pointer "
                : "blue logo flex items-center cursor-pointer"
            }`}
          >
        ...
        <div
          className={`Menu ${setMode ? "darkbg" : "lightbg"}    ${
            menu ? "openMenu" : ""
          }`}
        >
              ...
              item === "Home" ? (
                <a key={item} href="/">
                  <li
                    className={
                      setMode
                        ? " hover:text-purple-900 pb-5 text-white cursor-pointer transition duration-700 hover:animate-bounce flex"
                        : "hover:text-purple-900 pb-5 text-black cursor-pointer transition duration-700 hover:animate-bounce flex"
                    }
                  >
                    <FaDashcube /> {item}
                  </li>
