...
const Project: React.FC<ChildProps> = (props) => {
  ...
  return (
      ...
      <div className=" hover:border-purple-700 hover:border cursor-pointer  bg-darkMode-layer p-6 mb-10 rounded-xl flex items-center justify-center">
        <div className="">
 ...
 <div onMouseEnter={HandleShowText} onMouseLeave={HandleHideText} ><a href={props.giturl}><FaGithub /> </a> {text && <span className="absolute  sm:left-80   left-40 rounded-xl p-3 bg-darkMode-background text-darkMode-text text-sm"> View Github Site </span> }</div>
 <div onMouseEnter={HandleShowText2} onMouseLeave={HandleHideText2} ><a href={props.liveurl}> <FaLink /></a>{text2 && <span className="absolute sm:left-80   left-40 rounded-xl p-3 bg-darkMode-background text-darkMode-text text-sm">View Live Site</span> }</div>
          </div>
        </div>
      </div>
